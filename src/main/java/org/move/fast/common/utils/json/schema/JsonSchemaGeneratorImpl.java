...
public class JsonSchemaGeneratorImpl implements JsonSchemaGenerator {
    ...
    private final JsonSchemaConfig jsonSchemaConfig;
    ...
    private String fromJsonElement(JsonElement jsonElement) {
        ...
        JsonObject jsonSchemaElement = makeSchemaElement(jsonElement, null, true, null, null, null);
     ...
     * @return
     */
    public String json2Schema(String json, Map<String, String> nameAndNoteMap) {
        JsonElement jsonElement = null;
        try {
            jsonElement = validateInput(json);
        ...
        gsonBuilder.disableHtmlEscaping();
        Gson gson = gsonBuilder.create();
        JsonObject jsonSchemaElement = makeSchemaElement(jsonElement, null, true, null, nameAndNoteMap, null);
        return gson.toJson(jsonSchemaElement);
    }
     ...
     * @return
     */
    private JsonObject makeSchemaElement(JsonElement jsonElement, String elementName, boolean isFirstLevel, JsonArray required, Map<String, String> nameAndNoteMap, String key) {
...
//		}
        ...
        if (jsonSchemaConfig.isPrintDescription()) {
            ...
            jsonSchemaObject.addProperty(JsonSchemaKeywords.DESCRIPTION.toString(), nameAndNoteMap.get(key));
        }
        ...
        if (jsonElementType.equals(JsonValueTypes.OBJECT.toString()) && !jsonElement.getAsJsonObject().entrySet().isEmpty()) {// object.properties
            ...
            for (Map.Entry<String, JsonElement> propertyElemement : jsonElement.getAsJsonObject().entrySet()) {
                propertiesObject.add(propertyElemement.getKey(), makeSchemaElement(propertyElemement.getValue(), propertyElemement.getKey(), false, newRequired, nameAndNoteMap, propertyElemement.getKey().replaceAll("\\p{P}", "").toLowerCase()));
            }
            ...
            jsonSchemaObject.add(JsonSchemaKeywords.ITEMS.toString(), makeSchemaElement(jsonArray.get(0), "0", false, new JsonArray(), nameAndNoteMap, null));
        }
